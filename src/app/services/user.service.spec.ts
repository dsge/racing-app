...
import { AuthChangeEvent, Session, Subscription, SupabaseClient, User } from '@supabase/supabase-js';
  ...
  describe('getUser', () => {
    describe('internal unsub', () => {
      ...
      beforeEach(() => {
        ...
        supabaseClient.auth.onAuthStateChange = jasmine.createSpy().and.callThrough().and.returnValue({
          data: {
      ...
      it('should unsubscrube from the internal supabase auth change event after a user subscription suceeds', () => {
        ...
        expect(authStateChangeSubscription.unsubscribe).toHaveBeenCalledTimes(1);
      })
    })
    ...
    describe('user', () => {
      ...
      beforeEach(() => {
        supabaseClient = TestBed.inject(SUPABASE_CLIENT);
        supabaseClient.auth.onAuthStateChange = jasmine.createSpy().and.callFake((callback: (event: AuthChangeEvent, session: Session | null) => void | Promise<void>) => {
      ...
      it('should return the currently logged in user', (done: DoneFn) => {
        ...
        service.getUser().pipe(take(1)).subscribe((user: User | null) => {
    ...
    describe('isModerator()', () => {
      ...
      it('should return false when no user is logged in', (done: DoneFn) => {
        service.isModerator().pipe(take(1)).subscribe((isModerator: boolean) => {
      ...
      it('should return false when a user is not a moderator', (done: DoneFn) => {
        ...
        supabaseApiResponse$.next(new Response(JSON.stringify({
          data: [
            {
              is_moderator: false
            }
        ...
        authStateChangeCallback('SIGNED_IN', { user: { id: 'foo' } as User } as Session);
      })
    });
  })
});
