...
describe('UserVoteService', () => {
  ...
  describe('getUserVotes', () => {
    it('should return an empty array when no user is logged in', (done: DoneFn) => {
      ...
      service
        .getUserVotes({} as Race)
        .pipe(take(1))
        .subscribe((votes: UserVote[]) => {
    ...
    it('should return a list of votes when the user has voted for a race', (done: DoneFn) => {
      userService.getUser = jasmine
        .createSpy()
        .and.returnValue(of({ id: 'fakeuserid11' } as User));
      driverService.getDriverById = jasmine
        .createSpy()
        .and.returnValue(of({} as Driver));
      service
        .getUserVotes({ id: 555 } as Race)
        .pipe(take(1))
        .subscribe((votes: UserVote[]) => {
  ...
  describe('setUserVotes', () => {
    it('should return null when called without a logged in user', (done: DoneFn) => {
      userService.getUser = jasmine.createSpy().and.returnValue(of(null));
      service
        .setUserVotes({} as Race, [])
        .pipe(take(1))
        .subscribe((value: PostgrestSingleResponse<unknown> | null) => {
    ...
    it('should remove previous votes', () => {
      userService.getUser = jasmine
        .createSpy()
        .and.returnValue(of({ id: 'foo1122' } as User));
